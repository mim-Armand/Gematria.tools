import { Gematria } from '../src/gematria';
...
describe('Gematria', () => {
  ...
  it('calculates the gematria of Hebrew words correctly in ordinal style', () => {
    ...
    expect(Gematria.calculate('שבת', 'hebrew', 'ordinal')).toBe(45);
  });
  ...
  it('calculates the gematria of English words correctly in reduced style', () => {
    ...
    expect(Gematria.calculate('GEMATRIA', 'english', 'reduced')).toBe(38);
  });
  ...
  it('calculates the gematria of words using a custom alphabet', () => {
    ...
    expect(Gematria.calculate('ABC123', 'custom', 'standard', customAlphabet)).toBe(21);
  });

  it('throws an error for unsupported languages without custom alphabet', () => {
    expect(() => Gematria.calculate('ABC', 'unsupported')).toThrow('Unsupported language');
  });
  ...
  it('calculates gematria for sentences with a mix of supported languages and custom alphabets', () => {
    ...
    expect(Gematria.calculateSentence('אבג XYZ שבת', 'hebrew')).toBe(777 + 6); // 708 (hebrew) + 6 (custom)
  });
  ...
  it('throws an error if language cannot be detected and no custom alphabet is provided', () => {
    expect(() => Gematria.calculate('123')).toThrow('Language could not be detected and no custom alphabet was provided');
  });
});
